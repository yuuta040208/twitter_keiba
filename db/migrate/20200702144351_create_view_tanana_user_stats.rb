  ...
  def up
    ActiveRecord::Base.connection.execute('
      ...
      SELECT
        ...
        (coalesce(users.total_win_tanana, 0) / case when count(forecasts.tanana) = 0 then 1 else count(forecasts.tanana) end) as return_rate_win,
        (coalesce(users.total_place_tanana, 0) / case when count(forecasts.tanana) = 0 then 1 else count(forecasts.tanana) end) as return_rate_place,
        (
          select round(count(sub_f) * 1.0 / case when count(forecasts.tanana) = 0 then 1 else count(forecasts.tanana) end, 2) * 100 from forecasts sub_f
